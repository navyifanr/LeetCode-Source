...
public class Solution {
    //方法 1，暴力遍历， 时间复杂度 O(M*N) 空间复杂度 O(1)
    ...
    //方法 2，统计长度，从离终点相同距离开始比对，时间复杂度 O(M+N) 空间复杂度 O(1)
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        if (headA == null || headB == null) {
            return null;
        }
        ...
        curB = headB;
        if (lenA > lenB) {
            while (dis-- > 0) curA = curA.next;
        } else if (lenA < lenB) {
            while (dis-- > 0) curB = curB.next;
        }
        // 注意此处不是接上面的 else
        while (curA != null) {
            if (curA == curB) {
    ...
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        ...
        ListNode curA = headA;
        ListNode curB = headB;
        while (curA != curB) { //最终，要不就是相遇，要不就都是 null
